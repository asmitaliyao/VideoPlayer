package com.example.videoplayer.thread;
...
import com.example.videoplayer.mytry.MyVideoPlayerView;
...
public class VideoDecodeThread extends Thread {
    ...
    public void run() {
        ...
        for (int i = 0; i < mediaExtractor.getTrackCount(); i++) { // 信道总数
            Log.e(TAG, "run: mediaExtractor.getTrackCount() = "+ mediaExtractor.getTrackCount());
            MediaFormat format = mediaExtractor.getTrackFormat(i); // 音频文件信息
            mimeType = format.getString(MediaFormat.KEY_MIME);
            Log.e(TAG, "run: mimeType = " + mimeType );
            if (mimeType.startsWith("video/")) { // 视频信道
                ...
                while (!MyVideoPlayerView.isCreate){
                    try {
                        Thread.sleep(100);
